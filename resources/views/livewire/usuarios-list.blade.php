...
<div class="hidden lg:flex justify-between items-center gap-4">
    ...
    <input type="text" wire:model.live="search" placeholder="Buscar por nombre o email..."
        class="border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 w-full sm:w-1/3 
...
<div class="flex flex-col sm:flex-row justify-between items-center gap-4 lg:hidden">
    ...
    <div class="bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200 p-3 mt-4 rounded-lg shadow-md">
        {{ session('error') }}
    </div>
    @endif
        ...
        <div wire:key="mobile-usuario-{{ $usuario->id }}" class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between mb-2">
                ...
                <div x-data="{ open: false }" class="relative inline-block text-left">
                    <div>
                        <button @click="open = !open" type="button" class="inline-flex justify-center rounded-md border border-gray-300 dark:border-gray-700 shadow-sm px-2 py-1 bg-white dark:bg-gray-700 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-indigo-500" id="menu-button-{{ $usuario->id }}" aria-expanded="true" aria-haspopup="true">
                            <i class="fas fa-ellipsis-v"></i>
                        </button>
                    </div>
                    <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="origin-top-right absolute right-0 mt-2 w-40 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none z-10" role="menu" aria-orientation="vertical" aria-labelledby="menu-button-{{ $usuario->id }}" tabindex="-1">
                        <div class="py-1" role="none">
                            <a href="#" wire:click="abrirModalEditar({{ $usuario->id }})" class="text-gray-700 dark:text-gray-200 block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700" role="menuitem" tabindex="-1">
                                <i class="fas fa-edit mr-2"></i> Editar
                            </a>
                            <a href="#" wire:click="abrirModalVer({{ $usuario->id }})" class="text-gray-700 dark:text-gray-200 block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700" role="menuitem" tabindex="-1">
                                <i class="fas fa-eye mr-2"></i> Detalles
                            </a>
                            
                            {{-- Condición para NO mostrar el botón si es 'Administrador' --}}
                            @unless($usuario->hasRole('Administrador'))
                                <a href="#" wire:click="confirmarEliminar({{ $usuario->id }})" class="text-red-600 block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700" role="menuitem" tabindex="-1">
    ...
    <div class="bg-white dark:bg-gray-800 w-full max-w-lg p-6 rounded-2xl shadow-xl overflow-y-auto max-h-[80vh] relative">
        ...
        <form wire:submit.prevent="guardar" class="space-y-4 mb-16">
            ...
            <div>
                ...
                <input type="file" wire:model="form.profile_photo" class="w-full border rounded-lg px-3 py-2 dark:bg-gray-700 dark:text-gray-200">
                @error('form.profile_photo') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror

                @if ($form->profile_photo)
                <div class="mt-2">
                    <p class="text-sm text-gray-500 dark:text-gray-300">Vista previa:</p>
                    <img src="{{ $form->profile_photo->temporaryUrl() }}" alt="Vista previa" class="h-24 w-24 rounded-full object-cover border">
                </div>
                @elseif ($modo === 'editar' && $usuario && $usuario->profile_photo_path)
                <div class="mt-2">
                    <p class="text-sm text-gray-500 dark:text-gray-300">Foto actual:</p>
                    <img src="{{ asset('storage/' . $usuario->profile_photo_path) }}" alt="Foto actual" class="h-24 w-24 rounded-full object-cover border">
                </div>
                @endif
            </div>
