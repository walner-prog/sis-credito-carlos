<div class="p-6 lg:p-12 bg-gradient-to-r from-blue-50 via-purple-50 to-pink-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 min-h-screen">
    ...
    {{-- Desktop Header (Visible on lg screens and up) --}}
    ...
    {{-- Mobile Header (Visible below lg screens) --}}
    <div class="lg:hidden" x-data="{ searchOpen: false }">
        <div class="flex justify-between items-center px-4 py-2">
            ...
            <div class="flex items-center gap-2 relative">
                <div x-show="searchOpen" x-transition.duration.300ms class="flex items-center bg-gray-100 dark:bg-gray-700 rounded-full px-2 py-1 transition-all absolute right-0">
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="Buscar..." class="bg-transparent text-gray-800 dark:text-gray-200 placeholder-gray-500 dark:placeholder-gray-400 border-none focus:ring-0 focus:outline-none w-32 transition-all">
                ...
                <button @click="searchOpen = true" x-show="!searchOpen" class="text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-100 transition">
    ...
    {{-- Tabla para Escritorio (Visible on lg screens and up) --}}
    <div class="overflow-x-auto mt-6 hidden lg:block">
        <table class="w-full border-collapse rounded-lg overflow-hidden shadow-md">
            ...
            <tbody class="bg-white dark:bg-gray-800">
                ...
                <tr wire:key="rol-{{ $rol->id }}" class="border-b dark:border-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-800 transition">
                    <td class="p-3">{{ $rol->name }}</td>
                    <td class="p-3">
                        <div class="flex flex-col sm:flex-row gap-2 justify-start sm:justify-end">
                            <button wire:click="abrirModalEditar({{ $rol->id }})" class="bg-yellow-500 hover:bg-yellow-600 text-white p-2 rounded-full shadow-lg hover:scale-110 transition-transform duration-200 flex items-center justify-center {{ $rol->name === 'Administrador' ? 'opacity-50 cursor-not-allowed' : '' }}" title="Editar" @if($rol->name === 'Administrador') disabled @endif>
                                <i class="fas fa-edit"></i>
                            </button>
                            <button wire:click="abrirModalVer({{ $rol->id }})" class="bg-indigo-500 hover:bg-indigo-600 text-white p-2 rounded-full shadow-lg hover:scale-110 transition-transform duration-200 flex items-center justify-center" title="Detalles">
                                <i class="fas fa-eye"></i>
                            </button>
                            <button wire:click="confirmarEliminar({{ $rol->id }})" class="bg-red-600 hover:bg-red-700 text-white p-2 rounded-full shadow-lg hover:scale-110 transition-transform duration-200 flex items-center justify-center {{ $rol->name === 'Administrador' ? 'opacity-50 cursor-not-allowed' : '' }}" title="Eliminar" @if($rol->name === 'Administrador') disabled @endif>
                ...
                @empty
                <tr>
                    <td colspan="2" class="p-4 text-center text-gray-500 dark:text-gray-400">No se encontraron roles</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    {{-- Tarjetas para Móvil (Visible below lg screens) --}}
    <div class="mt-6 grid grid-cols-1 gap-4 lg:hidden">
        @forelse ($roles as $rol)
        <div wire:key="mobile-rol-{{ $rol->id }}" class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between mb-2">
                <h4 class="font-bold text-gray-800 dark:text-gray-100">{{ $rol->name }}</h4>
                <div x-data="{ open: false }" class="relative inline-block text-left">
                    <button @click="open = !open" type="button" class="inline-flex justify-center rounded-md border border-gray-300 dark:border-gray-700 shadow-sm px-2 py-1 bg-white dark:bg-gray-700 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-indigo-500">
                        <i class="fas fa-ellipsis-v"></i>
                    </button>
                    <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="origin-top-right absolute right-0 mt-2 w-40 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
                        <div class="py-1">
                            <a href="#" wire:click="abrirModalEditar({{ $rol->id }})" class="text-gray-700 dark:text-gray-200 block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 {{ $rol->name === 'Administrador' ? 'opacity-50 cursor-not-allowed pointer-events-none' : '' }}">
                                <i class="fas fa-edit mr-2"></i> Editar
                            </a>
                            <a href="#" wire:click="abrirModalVer({{ $rol->id }})" class="text-gray-700 dark:text-gray-200 block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700">
                                <i class="fas fa-eye mr-2"></i> Detalles
                            </a>
                            <a href="#" wire:click="confirmarEliminar({{ $rol->id }})" class="text-red-600 block px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 {{ $rol->name === 'Administrador' ? 'opacity-50 cursor-not-allowed pointer-events-none' : '' }}">
...
<div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 px-4">
    <div class="bg-white dark:bg-gray-800 w-full max-w-4xl p-6 rounded-2xl shadow-xl overflow-y-auto max-h-[80vh] relative mb-16">
        ...
        <div class="flex flex-wrap gap-2">
            @forelse($rolVer->permissions as $perm)
                <span class="bg-blue-200 text-blue-800 dark:bg-blue-900 dark:text-blue-200 px-3 py-1 rounded-full text-sm font-medium">
                    {{ $perm->name }}
                </span>
            @empty
...
@endif
    ...
    <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-sm p-6 mx-4">
            ...
            <p class="text-gray-600 dark:text-gray-300 mb-6">¿Estás seguro de que deseas eliminar este rol? Esta acción no se puede deshacer.</p>
